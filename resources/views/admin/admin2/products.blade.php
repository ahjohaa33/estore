        ...
        <div class="row">
            <div class="col-12">
                <div class="content-header">
                    ...
                    <div>
                        <button class="btn btn-light rounded font-sm mr-5 text-body hover-up" data-bs-toggle="modal" data-bs-target="#addProductModal">Add New Product</button>
                        <button class="btn btn-md rounded font-sm hover-up" id="bulkDeleteBtn" style="display: none;">Delete Selected</button>
                    </div>
                </div>
            </div>
        </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="me-3" style="width: 60px;">
                                               @php
                                                ...
                                                " src="{{ asset('storage').'/'.$firstImage }}" class="img-fluid" alt="{{ $product->name }}">
                                            </div>
                                            <div>
                                    ...
                                    <td class="text-end">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <a class="dropdown-item text-danger" href="#" onclick="deleteProduct({{ $product->id }})">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="modal-content">
                ...
                <form id="addProductForm" enctype="multipart/form-data">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-9">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input class="form-control" type="number" step="0.01" name="offer_price">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label">Offer Duration</label>
                                                    <input class="form-control" type="datetime-local" name="offer_duration">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <div class="mb-4">
                                                            ...
                                                            <input class="form-control" type="number" name="in_stock" value="0">
                                                        </div>
                                                        <div class="mb-4">
                                                            <label class="form-label">Sale Count</label>
                                                            <input class="form-control" type="number" name="sale_count" value="0">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="mb-4">
                                                        ...
                                                        <div class="form-check mb-4">
                                                            <input class="form-check-input" type="checkbox" name="is_fav" value="1">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-9">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <small class="form-text text-muted">Enter colors as JSON array</small>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label">Sizes (JSON format)</label>
                                                    <textarea class="form-control" name="size" placeholder='["S", "M", "L"]' rows="2"></textarea>
                                                    <small class="form-text text-muted">Enter sizes as JSON array</small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light rounded font-sm text-body hover-up" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-md rounded font-sm hover-up" id="submitProductBtn">Add Product</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const categoryFields = {
            'Clothing': [
                { type: 'text', name: 'brand', label: 'Brand', required: false },
                { type: 'select-multiple', name: 'sizes', label: 'Available Sizes', options: ['XS', 'S', 'M', 'L', 'XL', 'XXL'], required: false },
                { type: 'select-multiple', name: 'colors', label: 'Available Colors', options: ['Red', 'Blue', 'Green', 'Black', 'White', 'Yellow'], required: false },
                { type: 'text', name: 'material', label: 'Material', required: false }
            ],
            'Electronics': [
                { type: 'text', name: 'brand', label: 'Brand', required: false },
                { type: 'text', name: 'model', label: 'Model', required: false },
                { type: 'text', name: 'warranty', label: 'Warranty Period', required: false },
                { type: 'textarea', name: 'specifications', label: 'Technical Specifications', required: false }
            ],
            'Home & Garden': [
                { type: 'text', name: 'dimensions', label: 'Dimensions', required: false },
                { type: 'text', name: 'material', label: 'Material', required: false },
                { type: 'text', name: 'weight', label: 'Weight', required: false },
                { type: 'textarea', name: 'care_instructions', label: 'Care Instructions', required: false }
            ],
            'Sports': [
                { type: 'text', name: 'brand', label: 'Brand', required: false },
                { type: 'select', name: 'sport_type', label: 'Sport Type', options: ['Running', 'Football', 'Basketball', 'Tennis', 'Swimming', 'Cycling'], required: false },
                { type: 'text', name: 'material', label: 'Material', required: false },
                { type: 'text', name: 'weight', label: 'Weight', required: false }
            ],
            'Books': [
                { type: 'text', name: 'author', label: 'Author', required: false },
                { type: 'text', name: 'publisher', label: 'Publisher', required: false },
                { type: 'text', name: 'isbn', label: 'ISBN', required: false },
                { type: 'number', name: 'pages', label: 'Number of Pages', required: false }
            ]
        };
        ...
        function toggleBulkDeleteButton() {
            const selectedCount = document.querySelectorAll('#productsTableBody input[type="checkbox"]:checked').length;
        ...
        document.getElementById('bulkDeleteBtn').addEventListener('click', function() {
            const selectedIds = [];
            document.querySelectorAll('#productsTableBody input[type="checkbox"]:checked').forEach(checkbox => {
                selectedIds.push(checkbox.value);
            });

            if (selectedIds.length > 0 && confirm(`Are you sure you want to delete ${selectedIds.length} products?`)) {
